...
static void writeRegister(ADS1xx5_I2C *i2c, uint8_t reg, uint16_t value){
	...
	HAL_I2C_Master_Transmit(i2c->hi2c,
			i2c->m_i2cAddress,
			pData, // START, DevAddress + Write, pData[0], pData[1], pData[size-1], STOP.
			3,
			10);
}
...
static uint16_t readRegister(ADS1xx5_I2C *i2c, uint8_t reg){
	HAL_I2C_Master_Transmit(i2c->hi2c,
			...
			10);
	uint8_t pData[2] = {0, 0};
	HAL_I2C_Master_Receive(i2c->hi2c,
			i2c->m_i2cAddress,
			pData,
			2,
			10);
...
static void ADSbegin(ADS1xx5_I2C *i2c){
	if (HAL_I2C_IsDeviceReady(i2c->hi2c,
			i2c->m_i2cAddress,
			10, 10) != HAL_OK){
 ...
 * // The ADC input range (or gain) can be changed via the following
 ...
 //                                                                ADS1015  ADS1115
 //                                                                -------  -------
 // ADSsetGain(GAIN_TWOTHIRDS);  // 2/3x gain +/- 6.144V  1 bit = 3mV      0.1875mV
 // ADSsetGain(GAIN_ONE);        // 1x gain   +/- 4.096V  1 bit = 2mV      0.125mV
 // ADSsetGain(GAIN_TWO);        // 2x gain   +/- 2.048V  1 bit = 1mV      0.0625mV (default)
...
uint16_t ADSreadADC_SingleEnded(ADS1xx5_I2C *i2c, uint8_t channel, adsGain_t gain){
	if (channel > 3){
		return 0;
	}
	...
	uint16_t config =
		ADS1115_REG_CONFIG_CQUE_NONE 			|   	// Disable the comparator (default val)
					...
					ADS1115_REG_CONFIG_DR_860SPS 	| 	// 8 samples per second
	...
	// Set single-ended input channel
		...
		writeRegister(i2c, ADS1115_REG_POINTER_CONFIG, config);

		// Wait for the conversion to complete
		HAL_Delay(i2c->m_conversionDelay);

		// Read the conversion results
		// Shift the 12-bit results to 4 bits for the ADS1115 from ADS1015 CONFIG.
		return readRegister(i2c, ADS1115_REG_POINTER_CONVERT) >> i2c->m_bitShift; // 0 for ADS1115
}
 ...
 */
int16_t ADSreadADC_Differential_0_1(ADS1xx5_I2C *i2c, adsGain_t gain) {
	// Start with default values
	uint16_t config =
	ADS1115_REG_CONFIG_CQUE_NONE 	|   	// Disable the comparator (default val)
	ADS1115_REG_CONFIG_CLAT_NONLAT 	|  	// Non-latching (default val)
	...
	// Read the conversion results
	uint16_t res = readRegister(i2c, ADS1115_REG_POINTER_CONVERT) >> i2c->m_bitShift;
	if (i2c->m_bitShift == 0) {
		return (int16_t) res;
	} else {
 ...
 * a signed value since the difference can be either positive or negative.
 */
int16_t ADSreadADC_Differential_2_3(ADS1xx5_I2C *i2c, adsGain_t gain) {
	// Start with default values
	uint16_t config =
	ADS1115_REG_CONFIG_CQUE_NONE 	|   	// Disable the comparator (default val)
	ADS1115_REG_CONFIG_CLAT_NONLAT 	|  	// Non-latching (default val)
...
void ADSstartComparator_SingleEnded(ADS1xx5_I2C *i2c, uint8_t channel, int16_t threshold) {
	...
	writeRegister(i2c, ADS1115_REG_POINTER_HITHRESH, threshold << i2c->m_bitShift);
